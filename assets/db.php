...
function conn($db=false){
    global $db_name; global $db_user; global $db_password;
    if(!$db){
        $db=$db_name;
    }
    ...
    if($db===$db_name){
        ...
        if (!isset($conn_paq)){    
            $conn_paq = new PDO("mysql:host=localhost;dbname=$db",$db_user,$db_password,[
                PDO::ATTR_PERSISTENT => true
            ]);
            $conn_paq -> setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
        }
        return $conn_paq;
    }else{
...
function wild_card_query($m){
    
    if (!isset($m['query'])){
        $m['query'] = "";
    }

    if (!isset($m['query_append'])){
        $m['query_append'] = "";
    }
    
    $qa = preg_split("/\s+/",trim($m['query']));
    $qas = real_list($qa);
    
    $query = "SELECT * FROM `$m[table_name]`";
    ...
    if (count($qas)>0){
        ...
        if (strlen(trim($m['query_append']))>0){
...
function wild_card_search($m){
    return db_fetch(wild_card_query($m));
}

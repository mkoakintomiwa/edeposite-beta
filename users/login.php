...
<script>$("#login-button").on("click",(function(e){e.preventDefault(),Sp();var c=$("#ico-login-form").formJSON();$.ajax({url:`${ajax}/check-user`,data:Object.assign(c,{login:"true"}),success:e=>{switch(jsonResponse(e).status){case"correct":window.location.href=encodedURL("",c);break;case"wrong":Swal.fire({icon:"error",html:"Wrong credentials, check and try again"})}}})}));</script>
